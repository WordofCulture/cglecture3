Shader "Unlit/WorldPosDebug"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Turn world position into a colourful, repeating pattern
            float3 WorldPosToColor(float3 wpos, float scale)
            {
                float3 p = wpos * max(scale, 1e-4); //avoid div-by-zero
                // repeat 0...1 with frac; abs() prevents negative coolors when offsetting
                float3 base = frac(p);
                // add thin grid lines near multiples to help see Scalefloat3 edge = step(0.98, frac(p));
